...
def extract_flood(green_band_fp: str, nir_band_fp: str, mtl_fp: str = None):
    """
    Parameters
    ----------
    ...
    if SENSOR == "landsat8":
        g_reflect = radiance_to_reflectance(array=g_array, band=3, metadata=metadata)
        nir_reflect = radiance_to_reflectance(
            array=nir_array, band=5, metadata=metadata
        )
    ...
    filepath = os.path.join(
        OUTPUT,
        f'{dt.datetime.now().strftime("%Y%m%d%H%M%S")}-{SENSOR}-{LOCATION}-{EVENT}-{ALGORITHM}.tif',
    )
...
def extract_true_color(blue_band_fp: str, green_band_fp: str, red_band_fp: str):
    r_img = utils.load_image(fpath=red_band_fp)
    g_img = utils.load_image(fpath=green_band_fp)
    b_img = utils.load_image(fpath=blue_band_fp)

    r_array, r_profile, _ = utils.image_to_array(img=r_img)
    g_array, g_profile, _ = utils.image_to_array(img=g_img)
    b_array, b_profile, _ = utils.image_to_array(img=b_img)
    ...
    with rio.open(
        fp=filepath,
        mode="w",
        width=b_profile["width"],
        height=b_profile["height"],
        crs=b_profile["crs"],
        transform=b_profile["transform"],
        count=3,
        dtype=b_profile["dtype"],
    ) as tif:
        tif.write(b_array, 1)
        tif.write(g_array, 2)
        tif.write(r_array, 3)
